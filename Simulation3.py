...
from show import show_graph, my_plt, two_plt
import numpy as np
import torch

"""3 students succeed in 2 concepts, the rating r_2,2=1
...
if __name__ == "__main__":
    ...
    for _ in range(5):
        ...
        n = 100
        result = torch.zeros(n, 2, 2)
        for i in range(n):
            r1.train_now(train=train, nound=10)
            student_one_hot = torch.Tensor([[0, 1, 0, 0], [0, 0, 1, 0]])
            result[i, :, :] = r1.embedding_student(student_one_hot).data
        xs.append(result)
    two_plt(xs)
